...
(function (global) {
  ...
  function validateFieldsTop(obj, elements, path, issues, openEnded, strict, schemaComplete, opts) {
    ...
    for (const key of Object.keys(obj)) {
      ...
      if (!match) {
        // Every FHIR element (base type "Element") universally allows id and
        // extension, whether or not a specific complex-type/BackboneElement
        // schema here explicitly lists them - so these are never "unknown".
        if (key === 'id' || key === 'extension' || key === 'modifierExtension') continue;
        if (!openEnded) {
