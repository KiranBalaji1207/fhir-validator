...
(() => {
  ...
  validateOfflineBtn.addEventListener("click", async () => {
    ...
    // Remove the heuristic "no errors found" success message for now — we'll add a
    // combined one after the real schema check finishes (avoids a misleading early
    // "looks valid" badge before the authoritative check has run).
    result.issues = result.issues.filter(i => i.severity !== "success");

    renderOfflineResults(result);

    if (result.parseError) return; // nothing to schema-check if the JSON itself is broken

    resultsBody.insertAdjacentHTML("afterbegin", `<div class="online-note" id="schemaLoadingNote"><span class="spinner"></span>Running real FHIR base-schema validation…</div>`);
    ...
    if (parsedResource) {
      ...
      if (result.issues.filter(i => i.severity === "error").length === 0 && !result.issues.some(i => i.severity === "success")) {
