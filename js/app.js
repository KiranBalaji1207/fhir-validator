(function () {
  ...
  const vlDownloadBtn = document.getElementById('vl-download-btn');
  const vlCopyBtn = document.getElementById('vl-copy-btn');
  const vlCommand = document.getElementById('vl-command');
  const vlFilename = document.getElementById('vl-filename');
  const vlIgNote = document.getElementById('vl-ig-note');
  ...
  function runValidation() {
    ...
    renderResult(result);
    updateValidatorCommand(resourceObj, format);
  }
  ...
  // ---- Cross-check with the official validator (local) ----
  const IG_PACKAGE_MATCHERS = [
    { test: (url) => url.includes('/us/core/'), pkg: 'hl7.fhir.us.core' },
    { test: (url) => url.includes('/us/carin-bb/'), pkg: 'hl7.fhir.us.carin-bb' },
    { test: (url) => url.includes('/us/davinci-pdex/'), pkg: 'hl7.fhir.us.davinci-pdex' },
    { test: (url) => url.includes('/us/davinci-hrex/'), pkg: 'hl7.fhir.us.davinci-hrex' }
  ];

  function updateValidatorCommand(resourceObj, format) {
    const ext = format === 'xml' ? 'xml' : 'json';
    const filename = `fhir-resource.${ext}`;
    vlFilename.textContent = filename;

    const profiles = (resourceObj && resourceObj.meta && Array.isArray(resourceObj.meta.profile)) ? resourceObj.meta.profile : [];
    const packages = new Set();
    profiles.forEach((url) => {
      if (typeof url !== 'string') return;
      IG_PACKAGE_MATCHERS.forEach((m) => { if (m.test(url)) packages.add(m.pkg); });
    });

    let cmd = `java -jar validator_cli.jar ${filename} -version 4.0.1`;
    packages.forEach((pkg) => { cmd += ` -ig ${pkg}`; });
    vlCommand.textContent = cmd;

    if (packages.size) {
      vlIgNote.hidden = false;
      vlIgNote.textContent = `Detected profile(s) pointing at ${Array.from(packages).join(', ')} - added as -ig flags so the validator downloads and checks against them (needs internet access on your machine the first time, to fetch the package).`;
    } else {
      vlIgNote.hidden = true;
    }
  }

  vlDownloadBtn.addEventListener('click', () => {
    const text = inputArea.value;
    if (!text.trim()) return;
    const filename = vlFilename.textContent || 'fhir-resource.json';
  ...
  vlCopyBtn.addEventListener('click', () => {
    navigator.clipboard.writeText(vlCommand.textContent).then(() => {
      const original = vlCopyBtn.textContent;
      vlCopyBtn.textContent = 'Copied!';
      setTimeout(() => { vlCopyBtn.textContent = original; }, 1200);
    }).catch(() => {});
  });

  // Seed the command/filename display before any validation has run.
  updateValidatorCommand(null, 'json');
})();
