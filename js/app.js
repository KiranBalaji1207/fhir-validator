...
(() => {
  ...
  function groupLabel(source) {
    ...
    if (source === "schema") return "✅ Official FHIR base schema (real, spec-based validation)";
    if (source === "offline") return "Heuristic sanity checks (non-normative)";
    if (source && source.startsWith("ig:")) return escapeHtml(source.slice(3)) + " checks (heuristic)";
  ...
  function renderOfflineResults(result) {
    ...
    const order = Object.keys(groups).sort((a, b) => {
      const rank = (s) => (s === "syntax" ? 0 : s === "schema" ? 1 : s === "offline" ? 2 : s.startsWith("ig:") ? 3 : 4);
  ...
  validateOfflineBtn.addEventListener("click", async () => {
    ...
    resultsBody.insertAdjacentHTML("afterbegin", `<div class="online-note" id="schemaLoadingNote"><span class="spinner"></span>Running real FHIR base-schema validation (local, no network calls to third parties)…</div>`);

    let parsedResource;
    try { parsedResource = JSON.parse(text); } catch (e) { parsedResource = null; }

    if (parsedResource) {
      const schemaResult = await SchemaValidator.validate(parsedResource);
      const loadingNote = document.getElementById("schemaLoadingNote");
      if (loadingNote) loadingNote.remove();

      if (schemaResult.error) {
        resultsBody.insertAdjacentHTML("afterbegin", `
          <div class="online-note">
            <strong>Real schema-based validation isn't set up yet.</strong><br/>
            ${escapeHtml(schemaResult.error)}
          </div>`);
      } else if (schemaResult.issues.length) {
        result.issues.push(...schemaResult.issues);
      } else {
        result.issues.push({ severity: "success", message: "Passed real validation against the official FHIR base schema (structure, cardinality, data types, required elements). Note: this still does not check terminology bindings, slicing, or FHIRPath invariants from specific IGs/profiles.", path: "", source: "schema" });
      }

      if (result.issues.filter(i => i.severity === "error").length === 0 && !result.issues.some(i => i.source === "offline" && i.severity === "success")) {
        result.issues.push({ severity: "success", message: "No structural errors found by the heuristic checks.", path: "", source: "offline" });
      }
  ...
  // (confirmed directly from the project's own http-client-tests fixtures), and returns:
  //   { "outcomes": [ { "issues": [ { "line", "col", "message", "type", "level" } ] } ] }
  // This makes a real cross-origin API call to a third-party domain, which is the check
  // most likely to be blocked by restrictive corporate firewalls/proxies — if that's your
  // situation, rely on the offline schema-based check above instead (zero third-party calls).
    ...
    try {
      ...
      resultsBody.innerHTML = `
        <div class="online-note">
          ...
          This can happen if your browser/network blocks the cross-origin request (common on
          corporate firewalls), you're offline, or the public service's API has changed.<br/><br/>
          <strong>Fallback options:</strong>
          <ol>
            <li>Use the <strong>offline checks</strong> button above — it makes zero third-party network calls and works on any network.</li>
