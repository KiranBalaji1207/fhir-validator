...
(function (global) {
  ...
  const GENERIC_CODE_FALLBACK = ['active', 'final', 'completed', 'unknown'];
  // Fallback keyed by field name alone (not full path) for common cross-resource
  // code fields whose full context path varies too much to enumerate exactly.
  const FIELD_NAME_HINTS = {
    use: ['official', 'usual', 'home', 'work'],
    system: ['phone', 'email'],
    comparator: ['<', '<=', '>=', '>'],
  ...
  function generatePrimitive(typeName, contextKey) {
    switch (typeName) {
      ...
      case 'code': return pick(CODE_HINTS[contextKey] || FIELD_NAME_HINTS[fieldNameOf(contextKey)] || GENERIC_CODE_FALLBACK);
  ...
  function generateComplex(typeName, contextKey, resourceType, depth) {
    ...
    for (const el of def.elements) {
      maybeAssign(obj, el, contextKey + '.' + el.name, resourceType, true, depth);
    }
    return obj;
  }
  ...
  function generateValueForElement(el, contextKey, resourceType, depth) {
    if (el.type === 'BackboneElement') {
      ...
      for (const sub of el.elements || []) {
        maybeAssign(nested, sub, contextKey + '.' + sub.name, resourceType, true, depth + 1);
      }
      return nested;
    }
    return generateValueFor(el.type, contextKey, resourceType, depth);
  }
