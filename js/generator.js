...
(function (global) {
  ...
  const { getRealExample, getBaseRealExample } = isNode ? require('./realExamples.js') : global;
  ...
  const TWO_SEGMENT_HINTS = {
    ...
    'text.status': ['generated', 'extensions', 'additional', 'empty']
  };
  ...
  const FIELD_NAME_HINTS = {
    ...
    language: ['en', 'en-US', 'es', 'fr', 'de']
  };
  // A domain that (a) isn't one of the RFC 2606 reserved "example" domains real FHIR
  // validators specifically reject ("Example URLs are not allowed in this context"),
  // and (b) obviously isn't a real production system, for identifier.system,
  // Coding.system, Reference.type, and similar made-up URI/URL/canonical values.
  const TEST_DOMAIN = 'https://fhir-test-data.org';
  ...
  function generatePrimitive(typeName, contextKey) {
    if (fieldNameOf(contextKey) === 'size') return 10; // matches decoded length of the fixed base64Binary placeholder below
    switch (typeName) {
      ...
      case 'uri': case 'url': return `${TEST_DOMAIN}/${randomId()}`;
      case 'canonical': return `${TEST_DOMAIN}/StructureDefinition/${randomId()}`;
      case 'oid': return 'urn:oid:1.2.3.4.5.6.7.8';
      case 'uuid': return 'urn:uuid:' + randomUuid();
      case 'base64Binary': return 'U2FtcGxlRGF0YQ=='; // decodes to "SampleData" - 10 bytes, matches the 'size' special-case above
  ...
  function generateComplex(typeName, contextKey, resourceType, depth) {
    ...
    if (!def) return {};
    const obj = buildFromElements(def.elements, contextKey, resourceType, depth, true);
    return applyInvariantFixups(typeName, obj, contextKey);
  }

  // A handful of FHIR invariants and near-universal real-world expectations that
  // the generic "don't leave it empty" logic above doesn't know about:
  //   - Reference (ref-2): at least one of reference/identifier/display SHALL be present.
  //     A Reference left with only e.g. "type" set is structurally non-empty but still
  //     invalid, since ele-1-style emptiness checks don't know about ref-2 specifically.
  //   - ContactPoint (cpt-2): a system is required whenever a value is provided.
  //   - Identifier: system/value are both optional in base FHIR, but are almost always
  //     both present in real data, and are mandatory once profiles like US Core Patient
  //     are involved - so this generator always includes both rather than leaving it to chance.
  function applyInvariantFixups(typeName, obj, contextKey) {
    if (typeName === 'Reference') {
      const hasCore = ['reference', 'identifier', 'display'].some((k) => obj[k] !== undefined);
      if (!hasCore) obj.reference = stringFor(contextKey + '.reference');
    } else if (typeName === 'ContactPoint') {
      if (obj.system === undefined) obj.system = pick(FIELD_NAME_HINTS.system);
      if (obj.value === undefined) obj.value = stringFor(contextKey + '.value');
    } else if (typeName === 'Identifier') {
      if (obj.value === undefined) obj.value = stringFor(contextKey + '.value');
      if (obj.system === undefined) obj.system = `${TEST_DOMAIN}/identifiers/${randomId()}`;
    }
    return obj;
  }
  ...
  function generateResource(resourceType, options) {
    ...
    // Prefer a verbatim official HL7 example over synthetic generation, when one
    // exists for this profile - "generate it like the real IG example" beats a
    // randomized-but-schema-correct approximation.
    if (options.profileUrl) {
      const real = getRealExample(options.profileUrl);
      if (real && real.resourceType === resourceType) {
        let injected = null;
        if (options.injectError) {
          const elements = elementsForResource(resourceType);
          injected = injectRandomError(real, elements, null);
        }
        return { resource: real, injectedError: injected, isRealExample: true };
      }
    } else {
      // No profile selected ("Base FHIR R4 (no profile)") - if a verbatim,
      // no-profile FHIR R4 example is curated for this resource type (see
      // BASE_REAL_EXAMPLES in realExamples.js, sourced from the resource
      // examples HL7 publishes per https://hl7.org/fhir/R4/resourcelist.html),
      // use it instead of a randomized approximation.
      const baseReal = getBaseRealExample(resourceType);
      if (baseReal) {
        let injected = null;
        if (options.injectError) {
          const elements = elementsForResource(resourceType);
          injected = injectRandomError(baseReal, elements, null);
        }
        return { resource: baseReal, injectedError: injected, isRealExample: true };
      }
    }
