...
CarePlan, CareTeam, Composition, Provenance, RelatedPerson, Coverage,
ExplanationOfBenefit, Parameters, Consent, Bundle). For these, unrecognized
fields are **errors** regardless of the Strict mode toggle - the schema is
complete enough to be confident about it.
...
- **Da Vinci PDex**: the Prior Authorization ExplanationOfBenefit profile, the
  HRex/PDex Coverage profile, PDex Provenance, and the Provider/Payer Member
  Match request `Parameters` profiles. PDex's clinical data (Condition,
...
## Building a Member Match request (Da Vinci PDex)

The "Build a Member Match request" section builds the `Parameters` payload for
Da Vinci PDex's provider- or payer-initiated member match operation - the same
shape used by `$multi-member-match` (provider-initiated) and the payer-to-payer
bulk member match operation. Each "member" you add becomes a `MemberBundle`
parameter with `MemberPatient`, `CoverageToMatch`, an optional `CoverageToLink`,
and `Consent` parts, wired together with matching `Patient/<id>` references,
matching the structure HL7 publishes for this operation.

Fill in as many or as few fields as you have - anything left blank is omitted
rather than sent as an empty placeholder (empty FHIR elements aren't valid
structurally). If you leave out something FHIR actually requires (e.g.
`Coverage.payor`), the validator will tell you exactly which field is missing
once you generate the payload, the same way it would for anything else you
paste in.

## Cross-checking with the official HL7 validator

This tool is intentionally scoped to structural checks (shape, required fields,
cardinality, data types) - it can't check terminology bindings or FHIR invariants
like `ref-2` or `cpt-2`, because that requires the official HL7 FHIR validator
(`validator_cli.jar`), which is a **Java program**. Java bytecode can't run in a
browser or on a static site like this one, so there's no way to embed it here -
it has to run on your own machine.

The "Cross-check with the official HL7 FHIR validator" section bridges this
without breaking the "no external API calls, 100% static" design: it never runs
the validator itself, it just gives you an accurate, ready-to-run local command:

1. **Download `validator_cli.jar`** from the official
   [HL7 FHIR core releases](https://github.com/hapifhir/org.hl7.fhir.core/releases/latest)
   (requires Java 11+).
2. **Download the resource** currently in the editor as a file.
3. **Run the generated command** in the same folder. If the resource declares a
   `meta.profile` this tool recognizes (US Core, CARIN BB, Da Vinci PDex/HRex),
   the command automatically includes the matching `-ig` flag so the official
   validator checks against that implementation guide too.

Running the official validator this way does reach out to the internet on your
machine (to fetch IG packages and, unless you pass `-tx n/a`, to HL7's
terminology server for value-set checks) - that's a property of the official
tool itself, not of this site.

- **US Core** (9 → 12): Patient, Practitioner, Organization, Location, Encounter,
  Immunization, MedicationRequest, Observation (Vital Signs), AllergyIntolerance,
  **CareTeam**, **ServiceRequest**, and **QuestionnaireResponse** (the PRAPARE
  social-determinants-of-health example, including its nested item/answer
  structure) now use HL7's verbatim examples.
- **CARIN BB (Blue Button)**: Patient, Practitioner, Organization (Payer),
  RelatedPerson, Coverage, and all **five** ExplanationOfBenefit claim-type
  profiles (Inpatient, Outpatient, Pharmacy, Professional/NonClinician, Oral) -
  pulled from the IG's own [example instances](https://hl7.org/fhir/us/carin-bb/STU2.1/artifacts.html#7),
  also CC0-licensed.

On CareTeam specifically: US Core has included a CareTeam profile since STU1
(2016) through the current published version and beyond - it was missing from
this validator's profile list, not missing from US Core itself. That's now
fixed alongside adding its example.

## Real HL7 examples vs. synthetic generation

For 9 of the most commonly used US Core profiles, the generator uses the
**verbatim, official example resources HL7 publishes** on the
[US Core examples page](https://hl7.org/fhir/us/core/STU6.1/examples.html)
(CC0-licensed) instead of randomly generating data - selecting one of these
profiles in the generator gives you the exact same resource HL7 ships, every
time:

- Patient (`Patient-example` - "Amy Shaw")
- Practitioner (`Practitioner-practitioner-1` - "Dr Ronald Bone")
- Organization (`Organization-acme-lab`)
- Location (`Location-hospital`)
- Encounter (`Encounter-example-1`)
- Immunization (`Immunization-imm-1`)
- MedicationRequest (`MedicationRequest-self-tylenol`)
- Observation, Vital Signs (`Observation-heart-rate`)
- AllergyIntolerance (adapted from the FHIR core spec's canonical
  "Cashew nuts" AllergyIntolerance example, with the US Core profile added)

These interlink the same way the originals do (the Immunization references
the same `Patient/example`, `Encounter/example-1`, `Location/hospital`, and
`Practitioner/practitioner-1` HL7 uses), and the "include an intentional
error" option still works on them the same way it does on generated data.

Every other US Core profile - and every other resource type and IG in this
tool - still uses the schema-driven random generator described below, since
verifying a verbatim official example for every single profile is an ongoing
effort. If there's a specific profile you'd like added to the real-example
list, it's a small, well-defined addition.
...
js/realExamples.js   Verbatim official HL7 US Core examples (used when available)
