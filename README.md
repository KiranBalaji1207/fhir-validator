...
like `ref-2` or `cpt-2`. For that level of validation, use the official HL7 FHIR
validator (`validator_cli.jar`) separately.
...
Every other US Core profile - and every other IG-specific profile in this
...
### Base FHIR R4 examples (no profile)

When **"Base FHIR R4 (no profile)"** is selected instead of an IG profile, the
generator now also prefers a **verbatim FHIR R4 (v4.0.1) base-spec example**
over synthetic generation, for every resource type in this list that HL7
publishes one for on its own resource pages (see the
[R4 Resource Index](https://hl7.org/fhir/R4/resourcelist.html), which links to
each resource's `-examples` page):
...
Medication, MedicationRequest, DiagnosticReport, ServiceRequest, CarePlan,
CareTeam, Composition, Provenance, RelatedPerson, Coverage,
ExplanationOfBenefit, Consent, and Bundle.

These live in `js/realExamples.js` alongside the US Core/CARIN BB/Da Vinci PDex
ones (`BASE_REAL_EXAMPLES`, keyed by resource type instead of profile URL) and
have been checked against this validator's own structural schema so they come
back error-free out of the box. The "include an intentional error" option
still works on them the same way it does everywhere else. `Parameters` has no
single canonical base example (it's operation-specific) and continues to use
